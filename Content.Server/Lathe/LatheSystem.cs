...
{
    [UsedImplicitly]
    public sealed partial class LatheSystem : SharedLatheSystem // CS: add partial
    {
        ...
        public void UpdateUserInterfaceState(EntityUid uid, LatheComponent? component = null)
        {
            ...
            int? bufferAmount = null; // CS: Biomass buffer
            OnGetBufferAmount?.Invoke(uid, component, ref bufferAmount);  // CS: event to get buffer
            var state = new LatheUpdateState(GetAvailableRecipes(uid, component), component.Queue, producing, bufferAmount); // CS: add bufferAmount
