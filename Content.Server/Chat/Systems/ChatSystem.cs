...
public sealed partial class ChatSystem : SharedChatSystem
{
        ...
        )
    {
        ...
        chatColorSemiTransparent.A = 0.5f; // CS ADD - make the chat color semi-transparent, so it looks better
        ...
        var wrappedMessage = Loc.GetString(speech.Bold ? "chat-manager-entity-say-bold-wrap-message" : "chat-manager-entity-say-wrap-message",
            ...
            ("chatColor", chatColor ?? Color.White.ToHex())); // CS ADD - makes the your name color right
        ...
        var wrappedMessageSmall = Loc.GetString(speech.Bold ? "chat-manager-entity-say-bold-wrap-message" : "chat-manager-entity-say-wrap-message",
            ...
            ("verb", Loc.GetString(_random.Pick(speech.SpeechVerbStrings))),
            ("fontType", speech.FontId),
            ("fontSize", Convert.ToInt16(speech.FontSize * 0.7)), // CS ADD - shrunken by 20%
            ("message", FormattedMessage.EscapeText(message)),
            ("chatColor", chatColorSemiTransparentActually)); // CS ADD - makes the your name color right
        // CS ADD END
        ...
        SendInVoiceRange(
            ...
            source,
            range,
            voiceRange: floatRange, // CS ADD - shouts go further
            blockedByOcclusion: !SayGoesThroughWalls, // CS ADD - some things dont do thru walls
            ensmallenedByOcclusion: SayEffectedByOcclusion, // CS ADD - some things do get ensmallened by occlusion
    ...
    private void SendEntityEmote(
        ...
        string? chatColor = null // CS ADD - makes the your name color right
        )
    {
        ...
        SendInVoiceRange(ChatChannel.Emotes,
            ...
            range,
            author,
            voiceRange: EmoteRange, // CS ADD - emotes go further
            blockedByOcclusion: !EmotesGoesThroughWalls, // CS ADD - some things dont do thru walls
            ensmallenedByOcclusion: false); // CS ADD - emotes dont get ensmallened by occlusion
        if (!hideLog)
            if (name != Name(source))
    ...
    private void SendLOOC(EntityUid source, ICommonSession player, string message, bool hideChat)
    {
        ...
        var wrappedMessage = Loc.GetString("chat-manager-entity-looc-wrap-message",
            ("entityName", name),
            ("chatColor", nameColorString), // CS ADD - makes the your name color right
            ("message", FormattedMessage.EscapeText(message)));
        ...
        SendInVoiceRange(
            ...
            player.UserId,
            voiceRange: LOOCRange, // CS ADD - LOOC goes further
            blockedByOcclusion: !LOOCGoesThroughWalls, // CS ADD - some things dont do thru walls
            ensmallenedByOcclusion: false); // CS ADD - LOOC dont get ensmallened by occlusion
    ...
    private void SendSubtleLOOC(EntityUid source, ICommonSession player, string message, bool hideChat)
    {
        ...
        SendInVoiceRange(
            ...
            noGhosts: true); // CS ADD - Subtle LOOC does not go to ghosts, so we set noGhosts to true
        _adminLogger.Add(
            LogType.Chat,
            LogImpact.Low,
    ...
    private void SendInVoiceRange(
        ...
        ChatTransmitRange range,
        NetUserId? author = null,
        bool blockedByOcclusion = false, // CS ADD - some things dont do thru walls
        bool ensmallenedByOcclusion = false, // CS ADD - some things do get ensmallened by occlusion
        float voiceRange = 10f, // CS ADD - shouts go further
        string? occludedMessage = null,
        bool noGhosts = false) // CS ADD - some things do not go to ghosts
    {
        var numHeareded = 0;
        foreach (var (session, data) in GetRecipients(
                     source,
